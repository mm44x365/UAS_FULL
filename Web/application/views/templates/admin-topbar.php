    ...
    <div class="container">
      ...
      <div class="collapse navbar-collapse pull-left" id="navbar-collapse">
        <ul class="nav navbar-nav">
          ...
          <li class="<?= $title == 'Foto' ? 'active' : '' ?>"><a href="<?= base_url() ?>images/tambah">Input Foto</a></li>
          <?php if ($this->session->userdata('role') == 1 || $this->session->userdata('role') == 2) : ?>
            <li class="dropdown <?= $title == 'Data Pengguna' || $title == 'Data History'  || $title == 'Data Admin' ? 'active' : '' ?>">
              <a href="#" class="dropdown-toggle" data-toggle="dropdown"><?= $this->session->userdata('role') == '1' ? 'Menu Administrator' : 'Manajemen Data' ?> <span class="caret"></span></a>
              <ul class="dropdown-menu" role="menu">
                <?php if ($this->session->userdata('role') == 1) : ?>
                  <li class="<?= $title == 'Data Admin' ? 'active' : '' ?>"><a href="<?= base_url() ?>admins/list/">Data Admin</a></li>
                <?php endif; ?>
                <?php if ($this->session->userdata('role') == 2) : ?>
                  <li class="<?= $title == 'Data Pengguna' ? 'active' : '' ?>"><a href="<?= base_url() ?>users/list/">Data Pengguna</a></li>
